...
def run_test_pizza():
    ...
    title = 'PIZZA test 4:  6 slices, thin (thickness=2) red lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    circle.attach_to(window)
    window.render()
    generate_points_on_circle(circle, number_of_slices)
...
def run_test_polygon():
    ...
    title = ('POLYGON tests 3:  5 segments with thick blue lines;'
             + ' 8 with medium red lines.')
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(100, 100), 80)
    circle.outline_thickness = 3
    polygon(window, circle, 5, 'blue', 10)
    window.continue_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(number_of_segments-1):
        ...
        line.attach_to(window)
    window.render()
    line = rg.Line(list[len(list)-1],list[0])
    line.thickness = thickness
    line.color = color
    line.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    title = ('FANCY POLYGON test 4:  25 lime green lines on blue circle,'
             + ' hops = 10.')
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'blue'
    fancy_polygon(window, circle, 25, 10, 'lime green', 5)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(len(list)):
        line = rg.Line(list[(k+hops_to_next_point) % number_of_lines],list[k])
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
    window.render()
